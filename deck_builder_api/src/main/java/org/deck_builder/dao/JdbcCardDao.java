...
public class JdbcCardDao implements CardDao{
    ...
    public boolean addCardToDb(Card card){
        ...
        String insertSql = "INSERT INTO cards (card_name, scryfall_link, image_link, mana_cost, card_type, oracle_text, colors, " +
                "color_identity, keywords, scryfall_id, cmc, game_changer, rarity, full_art_link, back_side_card_name, back_side_card_type, " +
                "back_side_image, back_side_mana_cost, back_side_oracle_text, back_side_colors, two_card_faces)" +
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
        ...
        if (!cardExistsInDb) {
            jdbcTemplate.update(insertSql, card.getName(), card.getScryfallURL(), card.getImageLink(), card.getManaCost(), card.getType(),
                    ...
                    card.getGameChanger(), card.getRarity(), card.getFullArtLink(), card.getBackSideCardName(), card.getBackSideCardType(),
                    card.getBackSideImage(), card.getBackSideManaCost(), card.getBackSideOracleText(), card.getBackSideColors(), card.getTwoCardFaces());
            return true;
        }
        return false;
    }
